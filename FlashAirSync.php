...
$defaults = [
    ...
    'syncfrom' => '/DCIM',
    'syncto' => getenv('HOME') . '/Photos',
    'timezone' => 'GMT',
    'pingtimeout' => '5',
    'quiet' => false
];

$options = getopt('', ['flashairip::', 'syncfrom::', 'syncto::', 'timezone::', 'pingtimeout::', 'quiet::']) + $defaults;
...
$quiet = (boolean)$options['quiet'];          // Less versbosive
...
if (!alive()) {
    if (!$quiet) echo("Not Online\n");
    exit;
}

if (!$ForceUpdate && command(102) == 0) {
    if (!$quiet) echo "No Changes\n";
    exit;
}
